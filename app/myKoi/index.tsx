...
import { KoiFish, getMyKoiFishes } from "../../api/koi/myKoiApi";
...
const EmptyState = () => (
  <View style={styles.emptyStateContainer}>
    ...
    <Text style={styles.emptyStateText}>
      Bạn chưa có cá, hãy đăng ký cá của bạn
    </Text>
  </View>
);
...
export default function MyKoiPage() {
  ...
  const fetchMyFish = async () => {
    try {
      const response = (await getMyKoiFishes()) as {
        success: boolean;
        data: KoiFish[];
      };
  ...
  const fetchContestCount = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        const inContestCount = data.filter(
          (fish) => fish.contests && fish.contests.length > 0
        ).length;
  ...
  return (
    <>
      ...
      />
      <LinearGradient colors={["#eb7452", "#5C98BB"]} style={styles.container}>
