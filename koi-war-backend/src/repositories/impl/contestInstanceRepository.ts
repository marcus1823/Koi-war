...
export class ContestInstanceRepository implements IContestInstanceRepository {
    ...
    async getEndedContestInstances(): Promise<IContestInstance[]> {
        const now = new Date();
        return await ContestInstance.find({
            endDate: {$lt: now},
            isRanked: false
        })
    }

    async updateContestInstanceRankedStatus(id: string): Promise<IContestInstance | null> {
        const contestInstance = await ContestInstance.findByIdAndUpdate(id, {
            isRanked: true
        }, {new: true})
            .populate("contest")
